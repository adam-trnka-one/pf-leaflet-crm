  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center" data-testid="contacts-loading-container">
        <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-success" data-testid="contacts-loading-spinner"></div>
      </div>
    );
  }

  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-muted min-h-screen" data-testid="contacts-main-container">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6 sm:mb-8" data-testid="contacts-header-section">
        <div data-testid="contacts-header-content">
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground" data-testid="contacts-page-title">{t('contacts:title')}</h1>
          <p className="text-muted-foreground mt-1 sm:mt-2 text-sm sm:text-base" data-testid="contacts-page-subtitle">{t('contacts:subtitle')}</p>
        </div>
        <Button 
      ...
      <div className="mb-6" data-testid="contacts-search-section">
        <div className="relative" data-testid="contacts-search-container">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" data-testid="contacts-search-icon" />
          <Input
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-testid="contacts-grid">
        {filteredContacts.map((contact) => (
          <Link key={contact.id} to={`/dashboard/contacts/${contact.id}`} data-testid="contacts-contact-link">
            <Card className="bg-card shadow-sm hover:shadow-md transition-shadow cursor-pointer" data-testid="contacts-contact-card">
              <CardHeader className="pb-2" data-testid="contacts-card-header">
                <div className="flex items-center space-x-3" data-testid="contacts-contact-info-row">
                  <div className="w-12 h-12 bg-success-muted rounded-full flex items-center justify-center" data-testid="contacts-contact-avatar">
                    <User className="h-6 w-6 text-success-muted-foreground" data-testid="contacts-contact-avatar-icon" />
                  </div>
                  <div data-testid="contacts-contact-name-section">
                    <CardTitle className="text-lg font-semibold text-foreground" data-testid="contacts-contact-name">
                      {contact.firstName} {contact.lastName}
                    </CardTitle>
                    <p className="text-sm text-muted-foreground" data-testid="contacts-contact-title">{contact.title}</p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-3" data-testid="contacts-card-content">
                <div className="text-sm text-muted-foreground" data-testid="contacts-account-row">
                ...
                <div className="flex items-center text-sm text-muted-foreground" data-testid="contacts-email-row">
                  <Mail className="h-4 w-4 mr-2" data-testid="contacts-email-icon" />
                  <span data-testid="contacts-email-value">{contact.email}</span>
                </div>

                <div className="flex items-center text-sm text-muted-foreground" data-testid="contacts-phone-row">
                  <Phone className="h-4 w-4 mr-2" data-testid="contacts-phone-icon" />
                  <span data-testid="contacts-phone-value">{contact.phone}</span>
                </div>

                <div className="pt-2 border-t border-border" data-testid="contacts-metadata-section">
                  <div className="text-xs text-muted-foreground" data-testid="contacts-owner-row">
                    <span data-testid="contacts-owner-label">{t('common:owner')}: </span>
                    <span data-testid="contacts-owner-value">{contact.owner}</span>
                  </div>
                  <div className="text-xs text-muted-foreground" data-testid="contacts-created-row">
      ...
      {filteredContacts.length === 0 && (
        <div className="text-center py-12" data-testid="contacts-empty-state">
          <p className="text-muted-foreground" data-testid="contacts-empty-message">{t('contacts:noResults')}</p>
        </div>
      )}
